# Roadmap

## ~~v1.0 — Stabilization~~ (Completed)

- ~~Semantic versioning and first GitHub release~~
- ~~Public roadmap~~
- ~~Good first issues for community contributions~~

## ~~v1.1 — Raw SQL~~ (Completed)

- **`[TJoin]` attributes**: declarative JOIN support (3 overloads: simple, self-join with alias, chained) — no raw SQL needed for most multi-table queries
- **`TTContext.RawSelect<T>`**: for queries that attributes can't express (subquery, UNION, GROUP BY, aggregations) — maps raw SQL results to DTO classes via `[TColumn]`
- Both approaches: read-only, skip identity map, fully backward compatible

## ~~v2.0 — Audit Integrity, Directional JSon Attributes & Fail-Safe Startup~~ (Completed)

- **Change tracking and soft delete made trustworthy**: `Update` no longer writes the creation or deletion audit columns, an `UPDATE` no longer reaches a soft-deleted row, and a second `Delete` no longer overwrites the audit of the first
- **Directional JSon attributes**: `[TJSonIgnoreSerialize]` and `[TJSonIgnoreDeserialize]`, for columns that travel one way only
- **Fail-safe startup**: the HTTP server refuses to start on a configuration that cannot mean what it says - a protected route with no authentication class, an area that nobody could ever hold, a `BaseUri` carrying an address
- **Three more drivers**: InterBase, MariaDB and Oracle, bringing the count to seven
- **The suite runs on all seven dialects**, and the first run of the five that had never been executed found real defects
- **Currency, JWT RS256 with key rotation, algebraic filter expressions**: see the Delivered list below

## v2.1 — Commit Changes Event

- `TTContext.OnCommitChanges` — event fired after a transaction commit with a summary of all changes
- Each change carries: entity class, action (insert/update/delete), entity ID, old values, new values
- Enables event-driven replication, audit trails, and cache invalidation on external systems

## v2.2 — Batch Operations

- Native batch insert for data coming from external sources (file imports, system synchronization)
- Performance improvement for bulk operations

## v2.3 — Schema Awareness

- Schema diff: compare entity definitions against existing database schema
- Generate DDL scripts (CREATE/ALTER)

## Backlog

- Async support
- Query cache with invalidation
- HTTP enhancements (rate limiting, OpenAPI)

## Delivered

- **JWT RS256 and key rotation** - asymmetric token signing on OpenSSL `libcrypto` (issuer holds the private key, verifiers only the public one) plus the `kid` header, so keys can be rotated without invalidating tokens already issued
- **Algebraic filter expressions** - `TTProperty` / `TTExpression` with operator overloading, for grouped conditions such as `(A or B) and C` that the fluent chain cannot express
- **InterBase, MariaDB, Oracle drivers** — three additional FireDAC drivers (MariaDB chosen over MySQL: it has native `CREATE SEQUENCE`, which Trysil's client-assigned primary keys require)
